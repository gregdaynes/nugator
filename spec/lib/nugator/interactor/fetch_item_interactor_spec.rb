...
RSpec.describe Nugator::Interactor::FetchItem do
  ...
  context 'when request for items by id' do
    context 'with single id' do
      it 'returns an item by id' do
        ...
        expect(item.content).to be nil
      end

      it 'returns an full item' do
        request[:id] = [2]
    ...
    context 'when request for items by id' do
      ...
      it 'returns full items' do
        request[:id] = [1, 2]
        request[:content] = true

        items = subject

        expect(items.first.content).to eq 'Test Content'
        expect(items.last.content).to eq 'Test Content'
      end
